...
export default function HeroSlider({ events }: HeroSliderProps) {
  ...
  // ✅ Detect client-side safely (prevents hydration mismatch)
  useEffect(() => {
    setIsClient(true);
  }, []);

  // ✅ Set Swiper navigation once mounted
  useEffect(() => {
    if (
      swiperRef.current &&
  ...
  // Function remains outside render
              ...
              <SwiperSlide key={event._id}>
                <div className="relative bg-white overflow-hidden rounded-2xl shadow-md flex flex-col md:grid md:grid-cols-2 h-auto md:h-[340px] lg:h-[400px]">
                  ...
                  <div className="relative h-[200px] sm:h-[240px] md:h-auto flex items-center justify-center bg-gray-100">
                    ...
                    {event.store?.image && (
                      <div
                        className="absolute bottom-3 left-1/2 -translate-x-1/2 md:left-4 md:translate-x-0 bg-white rounded-full p-2 shadow-lg"
