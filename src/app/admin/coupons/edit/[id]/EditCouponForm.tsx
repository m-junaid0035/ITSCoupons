...
export default function EditCouponForm({
    ...
    const [coupon, setCoupon] = useState<any>(couponData);
    ...
    const [couponType, setCouponType] = useState(couponData?.couponType || "coupon");
    const [couponCode, setCouponCode] = useState(couponData?.couponCode || "");

    // Description
    const [descriptionHtml, setDescriptionHtml] = useState(couponData?.description || "");

    // Discount
    const [discount, setDiscount] = useState(couponData?.discount || "");

    // Coupon URL
    const [couponUrl, setCouponUrl] = useState(couponData?.couponUrl || "");

    // Store search
    const [storeSearch, setStoreSearch] = useState("");
    const [storeDropdownOpen, setStoreDropdownOpen] = useState(false);
    const [selectedStore, setSelectedStore] = useState<Store | null>(() => {
        const matched = storesData.find((s) => s._id === couponData?.storeId);
        if (matched) setStoreSearch(matched.name);
        return matched || null;
    });

    // ✅ handle formState changes
    useEffect(() => {
